...
def create_cluster_folders(cluster_data, source_folder_path, output_folder):
    ...
    for key, value in track(
        cluster_data.items(),
        description="📂 Transferring images to cluster folders",
        complete_style="red",
    ):
...
def create_cluster_grids(cluster_data, source_path, output_folder):
    ...
    for key, value in track(
        cluster_data.items(),
        description="📂 Writing cluster grid images",
...
def create_feature_dict(
    dataset_path, model, preprocess, model_type, n=10, save_path=".", device="cpu"
):
    """
    ...
    if os.path.exists(os.path.join(save_path, "feature_dictionary.pkl")):
        ...
        print(
            f"[yellow]Loaded existing feature dictionary with {len(feature_dict)} items."
        )
    else:
        print(
            f"[yellow]No existing feature dictionary found. Creating a new one at: {save_path}"
        )
    ...
    for idx, file in enumerate(
        track(
            file_list,
            total=len(file_list),
            description="Getting image features",
        ...
        if file.endswith(extensions):
            ...
            try:
                ...
                image_feature = extract_features(
                    image, model, preprocess, model_type, device
                )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Cluster images using ViT or ResNet and create grids."
    )
    parser.add_argument(
        "--image_dataset_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="./",
        help="Path to save the output grids (default: current directory).",
    )
    parser.add_argument(
        "--cluster_folder",
        type=str,
        default="./",
        help="Path to save the clustered images (default: current directory).",
    )
    parser.add_argument(
        "--feature_dict_path",
        type=str,
        default="./",
        help="Path to save/load the feature dictionary (default: current directory).",
    )
    parser.add_argument(
        "--num_clusters", type=int, default=5, help="Number of clusters."
    )
    parser.add_argument(
        "--use_feature_dict",
        action="store_true",
        help="Use existing feature dictionary instead of recalculating.",
    )
    parser.add_argument(
        "--model",
        type=str,
        choices=["vit", "resnet", "vgg16", "mobilenetv3", "clip"],
        default="vit",
    ...
    run_pipeline(
        ...
        use_feature_dict=args.use_feature_dict,
        model=args.model,
